...
- We spent quite a bit of time on `XHR` and &*callback* functions this semester, but did cover the other asynchronous alternatives. Modern APIs (and not just in the browser) utilize alternate paradigms such as the *Promise* object for asynchronous code, as well as `async` functions and the `await` keyword:
